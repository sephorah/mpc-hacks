...
| 15 | Cohort queue UI — in the provider queue, group cards sharing a `cohortId` under a collapsible cluster header ("N similar · med-renewal") with a distinct background tint; show a "N similar" badge on each card; queue still sorted by `createdAt` within each cluster | P3 | M | Séphorah | X |
| 16 | Cohort batch-close — "Close all [N] similar cases" button in the detail panel when `cohortId` present; confirmation dialog listing the case IDs; calls close logic for each | P3 | M | Séphorah | X |
