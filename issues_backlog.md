...
| 12 | Patient cohorting — add `cohortId: string \| null` to `Case` type; compute fingerprint (`type + sorted true-answer keys`) at intake; tag matching open cases with shared `cohortId`; seed data pre-populated with two med-renewal cases sharing a cohortId | P2 | M | Walid | |
